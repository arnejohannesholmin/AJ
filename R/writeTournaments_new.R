...
#'
#' @export
#' @rdname writeTournaments_new
#'
writeTournaments_new<-function(t="all",file=NULL){
	...
	if(length(file)==0){
		file =file.path(Sys.getenv("HOME"),"Documents/Diverse/Bordhockey/FantasyNBA/Handouts new/AllTournaments.xls")
		}
	
	
	##### Execution #####
	# Get the points acheived using the parameters specified:
	datT=get.points_fantasyNBA(dir,pow=c(3,3,2,1,1,1),w=c(32,32,31,28,20,14),t=t)$tournaments
