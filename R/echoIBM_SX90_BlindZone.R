...
#' \dontrun{}
#'
#' @importFrom cpplot3d SX90_nr0a
#' @importFrom echoIBM echoIBM echoIBM.addwarnings
...
#'
echoIBM_SX90_BlindZone <- function(
	events = NULL,
	dir = NULL, 
	project = NULL, 
	eventspec = NULL, 
	...
	if("SU90" %in% esnm){
		...
		system.time(warn <- catchWarnings(echoIBM(event=event, t=t, noise=noise, max.memory=2e9, parlist=list(pre=TRUE, seed="ind"), rand.sel=rand.sel[[esnm]], cores=cores, ...)))
		#    user   system  elapsed 
	...
	if("EK60" %in% esnm){
		...
		system.time(warn <- catchWarnings(echoIBM(event, t=t, noise=noise, max.memory=2e9, parlist=list(pre=TRUE, seed="ind"), max.radius=0.4, rand.sel=rand.sel[[esnm]], cores=cores, ...)))
		#   user  system elapsed 
